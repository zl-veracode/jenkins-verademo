...
API_ID=$(echo $VERACODE_SECRET_ID | cut -d "-" -f 2)

# Set the Veracode API SECRET
API_SECRET=$(echo $VERACODE_SECRET_ID_KEY | cut -d "-" -f 2)
...
API_PATH="/dae/api/core-api/webhook"
 ...
 signing_data=$VERACODE_WEBHOOK

 nonce="$(openssl rand -hex 16)"
 timestamp=$(date +%s"000")

 nonce_key=$(echo "$nonce" | xxd -r -p | openssl dgst -sha256 -mac HMAC -macopt hexkey:"$API_SECRET" | awk -F" " '{ print $VERACODE_SECRET_ID }')
 time_key=$(echo -n "$timestamp" | openssl dgst -sha256 -mac HMAC -macopt hexkey:"$nonce_key" | awk -F" " '{ print $VERACODE_SECRET_ID }')
 sig_key=$(echo -n "$VERACODE_API_VERSION" | openssl dgst -sha256 -mac HMAC -macopt hexkey:"$time_key" | awk -F" " '{ print $VERACODE_SECRET_ID }')
 signature=$(echo -n "$signing_data" | openssl dgst -sha256 -mac HMAC -macopt hexkey:"$sig_key" | awk -F" " '{ print $VERACODE_SECRET_ID }')
